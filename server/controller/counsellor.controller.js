...
export const getCounsellors = async (req, res) => {
    try {
        const [rows] = await pool.query("SELECT * FROM counsellor");
        const counsellors = rows;
        return res.status(200).json({
            success: true,
            message: "List Of All Counsellors",
            counsellors,
        });
    } catch (error) {
        console.log(error);
...
export const alertStudent = async (req, res) => {
    try {
        ...
        const subject = `Alert for ${student.name}`;
        const message = `Hello ${student.name},\n\nPlease note that you have very low activity points. Please participate in more events.\n\nBest regards,\nYour Counsellor`;

        await sendEmail(student.email, subject, message);

        res.status(200).json({ message: "Alert sent successfully", student });
    } catch (error) {
        console.error(error);
